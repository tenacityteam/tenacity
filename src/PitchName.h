...
SAUCEDACITY_DLL_API double FreqToMIDInote(const double freq);

SAUCEDACITY_DLL_API double MIDInoteToFreq(const double dMIDInote);
...
SAUCEDACITY_DLL_API unsigned int PitchIndex(const double dMIDInote);

// PitchOctave returns the octave index for a double dMIDInote note number,
// per result from FreqToMIDInote.
// MIDI note number 0 is C-1 in Scientific pitch notation.
SAUCEDACITY_DLL_API int PitchOctave(const double dMIDInote);
