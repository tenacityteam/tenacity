...
void EffectUIHost::OnHelp(wxCommandEvent & /* event */)
{
   if (mEffectUIHost.GetDefinition().GetFamily() == NYQUISTEFFECTS_FAMILY && (mEffectUIHost.GetDefinition().ManualPage().empty())) {
      ...
      HelpSystem::ShowHelp(FindWindow(wxID_HELP), mEffectUIHost.GetDefinition().HelpPage(), wxEmptyString, true);
   }
   else {
...
/* The following registration objects need a home at a higher level to avoid
 ...
 WaveTrack, like TenacityProject, has a registry for attachment of serializable
...
static ProjectFileIORegistry::ObjectReaderEntry projectAccessor {
   ...
   [](TenacityProject &project) { return &RealtimeEffectList::Get(project); }
};
