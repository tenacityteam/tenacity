...
bool ExportFFmpeg::InitCodecs(AudacityProject *project)
{
   ...
   mEncAudioCodecCtx->SetStrictStdCompliance(AUDACITY_FF_COMPLIANCE_EXPERIMENTAL);
...
bool ExportFFmpeg::Finalize()
{
   ...
   for (;;)
   {
      ...
      else
      {
         ...
         encodeResult = encode_audio(*mFFmpeg, mEncAudioCodecCtx.get(),
                                     pkt.get(), NULL, 0
                        );
      }
...
bool ExportFFmpeg::EncodeAudioFrame(int16_t *pFrame, size_t frameSize)
{
   ...
   while (mFFmpeg->av_fifo_size(mEncAudioFifo->GetWrappedValue()) >= nAudioFrameSizeOut)
   {
      ...
      if (pkt->GetDecompressionTimestamp() != AUDACITY_AV_NOPTS_VALUE)
      {
         pkt->RescaleDecompressionTimestamp(mEncAudioCodecCtx->GetTimeBase(), mEncAudioStream->GetTimeBase());      //wxLogDebug(wxT("FFmpeg : (%d) Writing audio frame with PTS: %lld."), mEncAudioCodecCtx->frame_number, (long long) pkt.pts);
      }
