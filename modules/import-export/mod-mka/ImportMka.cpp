...
/**********************************************************************
  ...
  Originally written by Steve Lhomme
  Modified by Avery King for Tenacity 1.4+
...
public:
    void PushTrackFrame(const AudioTrackInfo & tk, const binary *buf, uint32 bufsize) override
    {
        ...
        unsigned chn = 0;
        ImportUtils::ForEachChannel(*tk.importTrack, [&](auto& channel) {
            auto trackStart = (constSamplePtr)(buf) + tk.bytesPerSample * chn;
            channel.Append(trackStart, tk.mFormat, samples);
        });
    }
};

class MkaImportFileHandle final : public ImportFileHandleEx
{
public:
...
protected:
    ...
    ::FLAC__StreamDecoderWriteStatus write_callback(const ::FLAC__Frame *frame, const FLAC__int32 * const buffer[]) override
    {
        ...
        return GuardedCall< FLAC__StreamDecoderWriteStatus > ( [&] {
            ...
            unsigned chn = 0;
            ImportUtils::ForEachChannel(*currentTrack->importTrack, [&](auto& channel) {
                if (frame->header.bits_per_sample <= 16)
                {
                    ...
                    channel.Append(
                        (constSamplePtr)tmp.get(),
                        int16Sample,
                        frame->header.blocksize
                    );
                } else {
                    channel.Append(
                        (constSamplePtr)buffer[chn],
                        int24Sample,
                        frame->header.blocksize
                    );
                }
...
void MkaImportFileHandle::Import(
    ImportProgressListener& progressListener, WaveTrackFactory *trackFactory,
    TrackHolders &outTracks, Tags *tags, LabelHolders &labels,
    std::optional<LibFileFormats::AcidizerTags>&)
{
    BeginImport();
    ...
    for (auto &audioTrack : audioTracks)
    {
        ...
        {
            auto& waveTrack = audioTrack.importTrack;
            waveTrack = ImportUtils::NewWaveTrack(
                *trackFactory,
                audioTrack.mChannels,
                audioTrack.mFormat,
                audioTrack.mRate
            );
            ...
            {
                waveTrack->SetName(audioTrack.mName);
            }
        }
    }
    ...
    for (;;)
    {
        ...
        progressListener.OnImportProgress(
        ...
        if (!Cluster->CheckMandatory())
        {
            wxLogWarning(wxT("Matroska : Cluster in %s at %lld missing mandatory data, skipping"),
                             GetFilename(), Cluster->GetElementPosition());
    ...
    for (auto & tk : audioTracks)
    {
        if (tk.mSelected)
        {
            ...
            tk.importTrack->Flush();
            outTracks.push_back(std::move(tk.importTrack));
        }
    }
    ...
    progressListener.OnImportResult(
        IsStopped() ? ImportProgressListener::ImportResult::Stopped :
